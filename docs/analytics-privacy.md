...
visitors can opt out at any time via **Analytics settings**. Because nothing is
...
6. Set the shortest useful event-retention period. Ninety days is a reasonable
   default for this project. Once selected, state that exact period in the privacy
   policy.
...
2. Opting out through the notice or **Analytics settings** must stop further
   PostHog requests immediately.
3. After opting out and reloading, no PostHog request should fire.
4. Re-enabling through **Analytics settings** should resume `$pageview` events only.
