...
Plan 2's history-service, see review #4) and
...
So Plan 1 and Plan 2 compose, introduce one selector that both the provider and the
history-service consume, instead of Plan 1 exporting only a provider:
...
of the prod bundle. Plan 2's `makeHistoryService(getChartFetcher())` reuses the **same**
selector — no second source of truth for fetcher selection.
